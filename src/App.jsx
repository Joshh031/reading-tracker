...
export default function ReadingTracker() {
  ...
  const addBookToLibrary = async () => {
    ...
    const newBook = { ...bookForm, id: Date.now().toString() };
  ...
  const deleteBook = async (id) => {
    await saveLibrary(library.filter(b => b.id !== id));
  };
  };
      ...
      <div style={{ padding: "28px 24px 0" }}>
        ...
        {view === "library" && (() => {
          const filtered = library
            .filter(b => libraryFilter === "All" || b.format === libraryFilter || b.genre === libraryFilter)
            .filter(b => {
                ...
                <div className="card" style={{ marginBottom: 20, border: "1px solid #c8a96e44" }}>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 16 }}>
                    ...
                    <div>
                      ...
                      <input className="field-input" type="date" value={bookForm.dateFinished} onChange={e => setBookForm(f => ({ ...f, dateFinished: e.target.value }))} style={{ colorScheme: "dark" }} />
                    </div>
                    <div>
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 12 }}>
                    <div style={{ flex: 1 }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 4, flexWrap: "wrap" }}>
                        ...
                        <span style={{ fontFamily: "'DM Mono', monospace", fontSize: 9, letterSpacing: 2, color: book.format === "Physical" ? "#6a9a8a" : "#8a6a9a", border: `1px solid ${book.format === "Physical" ? "#2a4a3a" : "#3a2a4a"}`, padding: "2px 6px" }}>{book.format.toUpperCase()}</span>
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                        {book.rating > 0 && (
                          <span style={{ color: "#c8a96e", fontSize: 13, letterSpacing: 2 }}>{"★".repeat(book.rating)}{"☆".repeat(5 - book.rating)}</span>
                        )}
                        <span style={{ fontFamily: "'DM Mono', monospace", fontSize: 10, color: "#444", letterSpacing: 1 }}>{formatDate(book.dateFinished)}</span>
                      </div>
                    </div>
